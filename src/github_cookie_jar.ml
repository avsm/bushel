...
module Auth = struct
  ...
  let of_string s =
    ...
    let app_json = member "app" json in
    {
      token = member "token" json |> to_string;
      app = {
        name = member "name" app_json |> to_string;
        url = member "url" app_json |> to_string;
      };
      url = member "url" json |> to_string;
      id = member "id" json |> to_int;
      note = member "note" json |> to_option to_string;
      note_url = member "note_url" json |> to_option to_string;
    }
  ...
  let to_string t =
    let json = `Assoc (
      [
        ...
        "app", `Assoc [
          "name", `String t.app.name;
          "url", `String t.app.url;
        ];
        "url", `String t.url;
        "id", `Int t.id;
      ]
      @ (match t.note with None -> [] | Some note -> ["note", `String note])
      @ (match t.note_url with None -> [] | Some note_url -> ["note", `String note_url])
    ) in
...
let save ({ jar_path } as jar) ~name ~auth =
  ...
  fprintf auth_oc "%s" (Auth.to_string auth);
  close_out auth_oc;
